...
import { renderMessagesToDocx } from '../utils/exportRenderer.js'
...
type ExportOption = 'clipboard' | 'file' | 'docx'
export function ExportDialog({
	...
	const handleDocxSubmit = async () => {
		...
		try {
			const buf = await renderMessagesToDocx(messages, tools, columns)
			writeFileSync_DEPRECATED(filepath, buf)
			onDone({ success: true, message: `Conversation exported to: ${filepath}` })
		} catch (error) {
			onDone({
				success: false,
	...
	const options = [
		...
		},
		{
			label: 'Save to file',
			value: 'file',
			description: 'Save the conversation as a plain text file',
		},
		{
			label: 'Save as Word file',
			value: 'docx',
			description: 'Save the conversation as a Word (.docx) document',
		},
	]
